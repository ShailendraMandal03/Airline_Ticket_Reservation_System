...
def search_flights():
    ...
    if user=='economy':
        query =  "SELECT f.flight_no, f.from_location, f.to_location,f.date, f.arrival_date,f.departure_time, f.arrival_time, f.price_economy,f.seat FROM flight_detail f Join airline_details a ON f.Airline_id=a.Airline_id WHERE f.from_location=%s AND f.to_location=%s AND f.date=%s and a.active='Yes'"
        
    else:
        query =  "SELECT f.flight_no, f.from_location, f.to_location,f.date, f.arrival_date,f.departure_time, f.arrival_time, f.price_business,f.seat FROM flight_detail f Join airline_details a ON f.Airline_id=a.Airline_id WHERE f.from_location=%s AND f.to_location=%s AND f.date=%s and a.active='Yes'"
...
def get_ticket_details(customer_id):
    ...
    query="select t.pnr,t.flight_no,t.journey_date,f.from_location,f.to_location,t.class,t.no_of_passengers FROM ticket_details t JOIN flight_detail f ON t.journey_date=f.date AND t.flight_no=f.flight_no WHERE t.customer_id=%s AND t.booking_status='CONFIRMED'"
...
def ticket():
    ...
    return render_template('display_ticket.html', ticket_details=ticket_details)
...
def search_pnr():
    ...
    query = "select t.pnr,t.flight_no,t.journey_date,f.from_location,f.to_location,t.class,t.no_of_passengers,t.booking_status FROM ticket_details t JOIN flight_detail f ON t.journey_date=f.date AND t.flight_no=f.flight_no WHERE t.pnr=%s"
    cursor.execute(query,(pnr,))
    pnr_detail = cursor.fetchall()

    if pnr_detail:

        # query = "SELECT booking_status FROM ticket_details WHERE pnr = %s "
        # cursor.execute(query,(pnr,))
        # booking_status = cursor.fetchone()

        # print(booking_status)

        booking_status=pnr_detail[0][7]
...
def cancel_ticket(pnr):
    ...
    query = "SELECT 0.85 * p.payment_amount AS r_amount FROM Ticket_Details t JOIN Payment_Details p ON t.pnr = p.pnr WHERE t.pnr = %s"
    cursor.execute(query, (pnr,))
    ticket_info = cursor.fetchone()

    print(ticket_info)

    refund_amount = ticket_info['r_amount']
    print(refund_amount)
...
def A_addflight():
    if request.method == 'POST':
        ...
        query="INSERT INTO flight_detail VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        values=(flight_no,from_location,to_location,date,arrival_date,departure_time,arrival_time,price_economy,price_business,Airline_id,seat)
        cursor.execute(query,values)
